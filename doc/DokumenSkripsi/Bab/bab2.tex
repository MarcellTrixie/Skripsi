...
\begin{table}[H]
	...
	\begin{center}
		...
		digit & Angka desimal \\ 
		\hline 
		graph & Karakter cetak (kecuali spasi) \\ 
		\hline 
		lower & Huruf kecil \\ 
		\hline 
		print & Karakter cetak (termasuk spasi) \\ 
		\hline 
		punct & Karakter cetak (kecuali alfanumerik) \\ 
		\hline 
		space & \textit{White space} \\ 
		\hline 
		upper & Huruf kapital \\ 
		\hline 
		word & Karakter ''word'' \\ 
...
\begin{enumerate}
	...
	\item \textit{alpha} \\
	Kelas yang berisi dengan karakter huruf. Karakter-karakter yang termasuk yaitu, huruf a-Z atau A-Z.	Simbol atau karakter khusus tidak termasuk dalam kelas ini.
...
\textit{PDF Parser}~\cite{pdfparser} adalah sebuah \textit{PHP library} yang digunakan untuk mengekstrak data yang ada dalam sebuah file PDF. Fitur-fitur yang sudah ada dalam \textit{library} ini, yaitu:
