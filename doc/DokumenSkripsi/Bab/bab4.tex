...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
    	\Function{ExtractFromPDF}{\$pdf\_exctract}
    		...
    		\For{each sentence in $array$}
    			\State $row \gets$ Add $index$ with 1
				\State $pattern \gets$ Define regex /SKRIPSI/TUGAS AKHIR | Judul Bahasa Indonesia | Nama Lengkap | 10 digit NPM UNPAR | MATEMATIKA/FISIKA/TEKNIK INFORMATIKA | tahun/
        		\If{$pattern$ contain in $array$}
                	\State fill $result$ with errors report
            	\EndIf
        	\EndFor
